...
It's written entirely in JavaScript – the language of the web – and uses **WebRTC** for accurate peer-to-peer transport. No browser plugin, extension, or installation is required.

Using open web standards, WebTorrent connects website users to form a distributed, decentralized browser-to-browser network for efficient file transfer.
...
Browser-to-browser communication **cuts out the middle-man** and lets people communicate on their terms. No more client/server – just a network of peers, all equal. WebTorrent is the first step in the journey to [redecentralize][redecentralize] the Web.

> The way we code the web will determine the way we live online. So we need to bake # Frequently Asked Questions

## What is WebTorrent?

**WebTorrent** is the first torrent client that works in the **browser**. YEP, THAT'S RIGHT, THE BROWSER!

It's written entirely in JavaScript – the language of the web – and uses
**WebRTC** for accurate peer-to-peer transport. No browser plugin, extension, or installation is required.
...
content**. The more people that use a WebTorrent-powered website, the faster and more resilient it becomes.

Browser-to-browser communication **cuts out the middle-man** and lets people communicate on their terms. No more client/server – just a network of peers, all equal. WebTorrent is the first step in the journey to
...
Non-profit projects like [Wikipedia][wikipedia] and the [Internet Archive][archive] could reduce bandwidth and hosting costs by letting visitorschip in. Popular content is served browser-to-browser quickly and cheaply. Rarely accessed content is served reliably over HTTP from the origin server.

There are also exciting **business use cases**, from CDNs to app delivery.

> WebTorrent has significant business potential to change the traditional radically notion of client-server, with applications for internal infrastructure and external closed user communications. WebTorrent has moved from an "idea" to a science experiment to now on the edge of being viable. This is like really, seriously cool.
>
...
WebTorrent is still pretty new, but it's already being used in extraordinary ways:
...
The WebTorrent protocol works just like [BitTorrent protocol][bittorrent-protocol], except it uses [WebRTC][webrtc] instead of [TCP][tcp]/[uTP][utp] as the transport protocol.

To support [WebRTC's connection model][webrtc-signaling], we made a few changes to the tracker protocol. Therefore, a browser-based WebTorrent client or ** "web peer"** can only connect to other clients that support WebTorrent/WebRTC.

The protocol changes we made will be published as a
[BEP](http://www.bittorrent.org/beps/bep_0001.html). Until a spec is written, you can view the source code of the [`BitTorrent-tracker`][bittorrent-tracker] package.

Once peers are connected, the wire protocol used to communicate is exactly the same as in normal BitTorrent. This should make it easy for existing popular torrent clients like Transmission and uTorrent to add support for WebTorrent. **Vuze** [already has support][vuze-support] for WebTorrent!
...
To start using WebTorrent, include the
...
out of the box. Additional ways to access file content directly, including
a node-style stream, Buffer, or Blob URL.

Video and audio content can be streamed, i.e., playback will start before the entire
...
WebRTC [works everywhere][webrtc-everywhere], and browser support is excellent. **Chrome**, **Firefox**, and **Opera** for Desktop and Android, as well as **Microsoft Edge** and **Safari** have support.
...
## Can WebTorrent clients connect to regular BitTorrent clients?
...
In node.js, `webtorrent-hybrid` can download torrents from WebRTC peers or TCP peers (i.e., typical peers). You can use WebTorrent as a command-line program or programmatically as a node.js package.

To install `webtorrent-hybrid,` run the following command in your terminal (add the
`-I'm flag to install the command line program, omit it to install locally):

```
npm install webtorrent-hybrid -g
```

Note: If you need to use WebTorrent in the browser (where WebRTC is available
...
## Can WebTorrent clients on different websites connect?
...
client-to-server. However, browser-to-browser connections require the cooperation of both websites (i.e., the WebTorrent script must be present on both sites).

## Who builds WebTorrent?

WebTorrent was built by [Feross Aboukhadijeh][feross] and hundreds of open source
...
[PeerCDN][peercdn] was a next-generation CDN powered by WebRTC for efficient peer-to-peer delivery of website content. PeerCDN was founded by [Feross Aboukhadijeh][feross], [Abi Raja][abi], and [John Hiesey][jhiesey] in March 2013 and was sold to [Yahoo][yahoo] in December 2013.
...
valuable contributions are given committed access to the project to contribute as they
...
If you're looking for help getting started, join us in [Gitter][gitter] or on IRC at `#webtorrent` (freenode) and how you can get started.
...
first, most pieces will have approximately the same availability in the
network.
...
prioritize/deprioritize specific files in the torrent.
...
pieces. In other words, when sufficient media is buffered, we can use the standard
"rarest-first" piece selection algorithm.

But the fact is that with the speed of today's internet connections, the user is
going to finish downloading the torrent in a fraction of the time it takes to
...
It works well. Our goal with WebTorrent was to bring BitTorrent to the web
...
with existing clients and prevented adoption. The way we've done it, it is better. The
wire protocol is the same, but there's now a new way to connect to peers:
WebRTC, in addition to the existing TCP and uTP.

Also, re-inventing the protocol is a giant rabbit hole. There were already a lot of
risks when we started the project -- will WebRTC get adopted by all the browser
...
works really well, and we care more about building robust and helpful software than
...
live content and create a torrent for it. Viewers would follow this "feed" of torrent files (or info hashes) and download the content sequentially. Streamers would be around 10-20 seconds behind the live stream.
...
WebRTC data channels do not allow a website to discover your public IP address when there is a VPN in use. The WebRTC discovery process will find your VPN's IP address and the local network IP address.

Local IP addresses (e.g. 10.x.x.x or 192.168.x.x) can potentially be used to "fingerprint" your browser and identify across different sites that you visit, like a third-party tracking cookie. However, this is a separate issue from exposing your real public IP address, and it's worth noting that the browser already provides hundreds of vectors for fingerprinting you (e.g., your installed fonts, screen resolution, browser window size, OS version, language, etc.).

If you have a VPN enabled, then WebRTC data channels will not connect to peers using your actual public IP address, nor will it be revealed to the JavaScript running on the webpage.

At one point in time, WebRTC did have an issue where it would allow a website to discover your actual public IP address, but this was fixed a long time ago. Unfortunately, this unfortunate misinformation keeps bouncing around the Internet.

There's now a spec that defines precisely which IP addresses are exposed with WebRTC. If you're interested in further reading, you can read the [IP handling spec](https://tools.ietf.org/html/draft-ietf-rtcweb-ip-handling-01)
for yourself.
...
Streaming support depends on support for the `MediaSource` API in the browser. All
modern browsers have `MediaSource` support. In Firefox, support was added in Firefox 42 (i.e. Firefox Nightly).

[Many file types][render-media] are supported (again, depending on browser support), but only `.mp4`, `.m4v`, and `.m4a` have full support, including seeking.

To support video/audio streaming of arbitrary files, WebTorrent uses the [`videostream`][videostream] package, which in turn uses [`mp4box.js`][mp4box.js].cIf you think there may be a bug in one of these packages, please file an issue on the respective repository.
...
The protocol changes we made will be published as a [BEP](http://www.bittorrent.org/beps/bep_0001.html). Until a spec is written, you can view the source code of the [`BitTorrent-tracker`][bittorrent-tracker] package.
...
To start using WebTorrent, include the [`webtorrent.min.js`](https://cdn.jsdelivr.net/npm/webtorrent@latest/webtorrent.min.js)
script on your page. If you use [browserify](http://browserify.org/), you can `npm install webtorrent` and `require('webtorrent')`.
...
This supports video, audio, images, PDFs, Markdown, [and more][render-media], right
out of the box. Additional ways to access file content directly, including a node-style stream, Buffer, or Blob URL.

Video and audio content can be streamed, i.e., playback will start before the entire file is downloaded. Seeking works, too – WebTorrent dynamically fetches the needed torrent pieces from the network on-demand.
...
WebTorrent is an independent project started by [Feross Aboukhadijeh][feross] in October 2013. Unlike PeerCDN, **WebTorrent is free software**, licensed under the [MIT License][license]. You're free to use it however you like!

> "Free software" is a matter of liberty, not price. To understand the concept, you should think of "free" as in "free speech," not as in "free beer."
>
> <cite>— Richard Stallman, software freedom activist</cite>

On a technical level, PeerCDN and WebTorrent were built with different goals in mind. PeerCDN was optimized for low-latency downloads and fast peer discovery. This
meant the client and site owner trusted centralized servers to map file URLs to
content hashes.
...
Streaming support depends on support for the `MediaSource` API in the browser. All modern browsers have `MediaSource` support. In Firefox, support was added in Firefox 42 (i.e. Firefox Nightly).

[Many file types][render-media] are supported (again, depending on browser support), but only `.mp4`, `.m4v`, and `.m4a` have full support, including seeking.

To support video/audio streaming of arbitrary files, WebTorrent uses the [`videostream`][videostream] package, which in turn uses [`mp4box.js`][mp4box.js]. If you think there may be a bug in one of these packages, please file an issue on the respective repository.

[videostream](https://npmjs.com/package/), [mp4box.js](https://github.com/gpac/mp4box.js)

## Got more questions?

Open an issue on the WebTorrent [issue tracker][issues], or join us in [Gitter][gitter] or on IRC at `#webtorrent` (freenode).
