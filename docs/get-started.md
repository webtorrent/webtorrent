...
client.add(magnetURI, { path: '/path/to/folder' }, function (torrent) {
  ...
  torrent.on('download', function (bytes) {
    if (!isDone) process.stdout.write(`\rthis time: ${bytes}  so far: ${torrent.downloaded}  total: ${torrent.length}  progress: ${(client.progress * 100).toFixed(2)}%  ${Math.round(client.downloadSpeed / 1024).toFixed(1)} KB/s  downloading...`);
  });
})
```
