...
import { v4 } from 'default-gateway' // browser exclude
import queueMicrotask from 'queue-microtask'

const debug = Debug('webtorrent:nat-traversal')

const p = (fn, bind, ...args) => new Promise(resolve => fn.call(bind, ...args, resolve))
...
class NatTraversal {
  constructor () {
    ...
    // Lookup gateway IP
    v4().then(({ gateway }) => {
      if (!gateway) return debug('Could not find gateway IP for NAT-PMP')
  ...
  async _upnpPortMapping (port, protocol) {
    ...
    const err = await p(this._upnpClient.portMapping, this._upnpClient, {
      public: port,
      private: port,
  ...
  async _pmpPortMapping (port, protocol) {
    ...
    const err = await p(this._pmpClient.portMapping, this._pmpClient, {
      private: port,
      public: port,
      ttl: this.ttl,
      type: protocol
  ...
  async portUnMapping (port) {
    ...
    const errU = await p(this._upnpClient.portUnmapping, this._upnpClient, { public: port })
    ...
    const errN = await p(this._pmpClient.portUnmapping, this._pmpClient, { private: port, public: port })
