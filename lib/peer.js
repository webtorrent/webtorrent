...
class Peer extends EventEmitter {
  ...
  onConnect () {
    ...
    const wire = this.wire = new Wire(this.type, this.retries, false)
    ...
    if (this.swarm) {
      if (this.type === 'tcpOutgoing') {
        if (this.retries === 0 && !this.sentPe1) this.sendPe1()
  ...
  onPe3 (infoHashHash) {
    if (this.swarm) {
      if (sha1.sync(Buffer.from(Buffer.from('req2', 'utf8').toString('hex') + this.swarm.infoHash, 'hex')) !== infoHashHash) {
