...
var BitField = require('bitfield'),
  ...
  Rusha = require('rusha-browserify'), // Fast SHA1 (works in browser)
...
function Piece (index, hash, buffer) {
  var self = this;
  if (!(self instanceof Piece)){
    return new Piece(index, hash, buffer);
  }
...
Piece.prototype.readBlock = function (offset, length) {
  var self = this;
  return self._verifyOffset(offset)?self.buffer.slice(offset, offset + length):undefined;
};
...
Piece.prototype.selectBlock = function (endGame) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    return {
      ...
      length: (i === len - 1) ? self.length - (i * BLOCK_LENGTH) : BLOCK_LENGTH
    };
  }
  return null;
};
...
function File (file, buffer, pieces) {
  ...
  if (!(self instanceof File)){
    return new File(file, buffer, pieces);
  }
  EventEmitter.call(self);
...
function Storage (parsedTorrent) {
  ...
  if (!(self instanceof Storage)){
    return new Storage(parsedTorrent);
  }
  EventEmitter.call(self);
  ...
  self.pieces = self.parsedTorrent.pieces.map(function (hash, index) {
    var start = index * self.pieceLength,
      end = start + self.pieceLength,
      buffer = self.buffer.slice(start, end), // references same memory
      piece = new Piece(index, hash, buffer);
...
Object.defineProperty(Storage.prototype, 'numMissing', {
  get: function () {
    var self = this,
      numMissing = self.pieces.length,
      index;
    for (index = 0, len = self.pieces.length; index < len; index++) {
...
Storage.prototype.readBlock = function (index, offset, length) {
  ...
  return !piece ? null : piece.readBlock(offset, length);
};
...
Storage.prototype.selectBlock = function (index, endGame) {
  var self = this,
    piece = self.pieces[index];
  return !piece ? null : piece.selectBlock(endGame);
};
