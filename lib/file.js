...
class File extends EventEmitter {
  ...
  _serve (req) {
    ...
    const range = rangeParser(this.length, req.headers.range || '')
    const type = mime.getType(this.name)
    const res = File.getSWResponse(req, type, this.length, range)
  ...
  static getSWResponse (req, type, length, range) {
    const res = {
      status: 200,
      headers: {
    ...
    if (range.constructor === Array) {
      ...
      res.headers['Content-Range'] = `bytes ${range.start}-${range.end}/${length}`
