...
export default class File extends EventEmitter {
  ...
  deselect () {
    ...
    // Boundary pieces can be shared with adjacent files. Deselecting the whole
    // range would strip those pieces from the selection pool even though a
    // neighbouring file still needs them, leaving that file stuck one piece
    // short. Only deselect pieces no other selected file depends on.
    let start = this._startPiece
    let end = this._endPiece

    if (this._torrent.files.some(file => file !== this && !file._deselected && file.length !== 0 && file.includes(start))) {
      start++
    }
    if (end >= start && this._torrent.files.some(file => file !== this && !file._deselected && file.length !== 0 && file.includes(end))) {
      end--
    }
