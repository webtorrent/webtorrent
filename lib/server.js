...
class NodeServer extends ServerBase {
  ...
  listen (...args) {
    ...
    if (this.client.natTraversal != null) {
      this.server.on('listening', () => {
        try {
          this.client.natTraversal.map(this.server.address().port)
        } catch ( error ) {}
      })
    }
  ...
  close (cb = () => {}) {
    ...
    if (this.client.natTraversal != null && this.server.address()) {
      try{
        this.client.natTraversal.unmap(this.server.address().port)
      } catch ( error ) {}
    }
    super.close()
