...
const debug = debugFactory('webtorrent:server')
...
class NodeServer extends ServerBase {
  ...
  listen (...args) {
    ...
    if (this.client.natTraversal != null && this.nat === true) {
      this.server.on('listening', () => {
        try {
          const port = this.server.address().port
          this.client.natTraversal.map({ publicPort: port, privatePort: port, protocol: 'tcp', description: 'WebTorrent server' })
        } catch (err) {
          debug('error mapping WebTorrent server via UPnP/PMP: %o', err)
        }
      })
    }
  ...
  close (cb = () => {}) {
    ...
    if (this.client.natTraversal != null && this.nat === true && this.server.address()) {
      try {
        const port = this.server.address().port
        this.client.natTraversal.unmap({ publicPort: port, privatePort: port, protocol: 'tcp', description: 'WebTorrent server' })
      } catch (err) {
        debug('error unmapping WebTorrent server via UPnP/PMP: %o', err)
      }
    }
    super.close()
