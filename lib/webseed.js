...
export default class AbstractWebSeed {
  ...
  async init () {
    for (const file of this._torrent.files) {
      try {
        ...
        if (ok) this.backpressued ??= backpressured
      } catch (e) {
   ...
   */
  async fetch ({ file, range = undefined, method = 'GET', critical = false, controller = abortTimeout() }) {
    const url = this.urlTransform(
      this.origin +
      (this.origin[this.origin.length - 1] === '/' ? '' : '/') +
      file.path.replace(this._torrent.path, '')
    )

    const request = this.requestTransform({
      cache: 'no-store',
      method,
      headers: {
