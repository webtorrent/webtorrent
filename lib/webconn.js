  ...
  async httpRequest (pieceIndex, offset, length, cb) {
    ...
    try {
      chunks = await Promise.all(requests.map(async ({ start, end, url }) => {
        ...
        const extraHeaders = resolveWebSeedHeaders(this._torrent.client._webSeedHeaders, url)
...
function resolveWebSeedHeaders (webSeedHeaders, url) {
  if (!webSeedHeaders) return {}
  try {
    const headers = typeof webSeedHeaders === 'function'
      ? webSeedHeaders(url)
      : webSeedHeaders
    ...
    debug('webSeedHeaders hook failed: %s', err.message)
  }
  return {}
}
