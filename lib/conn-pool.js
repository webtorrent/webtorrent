...
export default class ConnPool {
  constructor (client) {
    ...
    this.tcpServer.listen(client.torrentPort, () => {
      ...
      if (this._client.utp) {
        ...
        this.utpServer.listen(tcpAddr.port, tcpAddr.address)
  ...
  _onConnection (conn, type) {
    ...
    const peer = type === 'utp'
      ? Peer.createUTPIncomingPeer(conn, this._client.throttleGroups, this._client.secure)
      : Peer.createTCPIncomingPeer(conn, this._client.throttleGroups, this._client.secure)

    const wire = peer.wire
    wire.once('crypto-infohash', onCryptoInfoHash)
    wire.once('handshake', onHandshake)

    async function onCryptoInfoHash (infoHashHash) {
      const torrent = await self._client._getByHash(infoHashHash)
      if (torrent) {
        peer.swarm = torrent
        torrent._addIncomingPeer(peer)
        await wire.setInfoHash(torrent.infoHash)
      } else {
    ...
    function cleanupPending () {
      ...
      wire.removeListener('crypto-infohash', onCryptoInfoHash)
...
ConnPool.UTP_SUPPORT = !!utp?.createServer
