...
export default class ConnPool {
  constructor (client) {
    ...
    // Start TCP
    this.tcpServer.listen(client.torrentPort, this._client.torrentHost, () => {
      ...
      if (this._client.utp) {
        ...
        this.utpServer.listen(this.tcpServer.address().port, IPv4Pattern.test(this._client.torrentHost) ? this._client.torrentHost : undefined) // uTP-native doesn't support IPv6 yet
      } else {
        this._onListening()
      }
    })
