...
export default class ConnPool {
   ...
   * Closes both TCP and UTP servers and cleans up all pending connections.
   * @param  {function} [cb] - Optional callback function to call when destruction is complete
   */
  destroy (cb) {
   ...
   * @param {net.Socket|object} conn - The incoming connection (TCP socket or UTP connection)
   * @param {'tcp'|'utp'} type - The connection type
   * @private
   */
