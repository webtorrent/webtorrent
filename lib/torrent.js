...
export default class Torrent extends EventEmitter {
  constructor (torrentId, client, opts) {
    ...
    this.storeBitfield = !!opts.storeBitfield
  ...
  getStoreBitfield (cb) {
    const fileName = path.join(this.path, this.name + ' - ' + this.infoHash.slice(0, 8) + '.bin')
    fs.readFile(fileName, (err, data) => {
      if (err) {
        return cb(null)
      }
      cb(new BitField(new Uint8Array(data.buffer)))
    })
  }

  saveStoreBitfield (cb) {
    const fileName = path.join(this.path, this.name + ' - ' + this.infoHash.slice(0, 8) + '.bin')
    fs.writeFile(fileName, this.bitfield.buffer, (err) => {
      if (err) {
        // Fail silently
        return cb(null)
      }
  ...
  _verifyPieces (cb) {
    if (this.storeBitfield) {
      this.getStoreBitfield((storedBitfield) => {
        if (this.destroyed) return cb(new Error('torrent is destroyed'))

        if (!storedBitfield) {
          this._debug('could not access bitfield from file system')
          this.storeBitfield = false;
          return this._verifyPieces((arg) => {
            this.storeBitfield = true;
            this.saveStoreBitfield(cb)
          });
        }

        let validPieces = 0;
        this.pieces.forEach((piece, index) => {
          if (storedBitfield.get(index)) {
      ...
      if (hex === self._hashes[index]) {
        ...
        self.store.put(index, buf, err => {
          if (err) {
            ...
            if (self.storeBitfield) {
              self.saveStoreBitfield(() => {})
            }
