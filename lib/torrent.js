...
class Torrent extends EventEmitter {
  ...
  async load (streams, cb) {
    ...
    // promisify this.store.put
    const put = async (index, chunk) => {
      return new Promise((resolve, reject) => {
        this.store.put(index, chunk, (err) => {
          if (err) reject(err)
          resolve()
        })
      })
    ...
    try {
      // collects data until there's a full chunk
      ...
      for (let stream of streams) { // load data from streams in order
        // can be lazy functions that return streams
        ...
        for await (const buffer of stream) {
          ...
          while (bytesQueue.length >= this.pieceLength) {
            // shift() one chunk from bytesQueue
            const newChunk = bytesQueue.slice(0, this.pieceLength)
            bytesQueue = bytesQueue.slice(this.pieceLength)

            const putPromise = put(chunkIndex, newChunk)
      ...
      // store any left over bytes in a final chunk (that isn't the full pieceLength)
      if (bytesQueue.length > 0) {
        const putPromise = put(chunkIndex, bytesQueue)
        putPromises.push(putPromise)
      }
