...
try {
  ...
  TMP = path.join(typeof os.tmpdir === 'function' ? os.tmpdir() : '/',
    'webtorrent')
}
...
class Torrent extends EventEmitter {
      ...
      function onResponse (err, res, torrent) {
        ...
        if (res.statusCode !== 200) {
          self.emit('warning', new Error(
            `non-200 status code ${res.statusCode} from xs param: ${url}`))
          return cb(null)
        }
        ...
        if (!parsedTorrent) {
          self.emit('warning',
            new Error(`got invalid torrent file from xs param: ${url}`))
          return cb(null)
        }

        if (parsedTorrent.infoHash !== self.infoHash) {
          self.emit('warning', new Error(
            `got torrent file with incorrect info hash from xs param: ${url}`))
          return cb(null)
        }
    ...
    if (this.skipVerify) {
      ...
      if (this._fileModtimes && this._store === FSChunkStore) {
        ...
        this.getFileModtimes((err, fileModtimes) => {
          ...
          const unchanged = this.files.map((_, index) => fileModtimes[index] ===
            this._fileModtimes[index]).every(x => x)
  ...
  addPeer (peer) {
    if (this.destroyed) throw new Error('torrent is destroyed')
    if (!this.infoHash) throw new Error(
  ...
  _onWireWithMetadata (wire) {
    ...
    if (wire.peerExtensions.dht && this.client.dht &&
  ...
  _updateWireWrapper (wire) {
    ...
    if (typeof window !== 'undefined' && typeof window.requestIdleCallback ===
      'function') {
  ...
  _updateWire (wire) {
    ...
    const minOutstandingRequests = getBlockPipelineLength(wire,
      PIPELINE_MIN_DURATION)
    if (wire.requests.length >= minOutstandingRequests) return
    const maxOutstandingRequests = getBlockPipelineLength(wire,
      PIPELINE_MAX_DURATION)

    trySelectWire(false) || trySelectWire(true)

    function genPieceFilterFunc (start, end, tried, rank) {
      return i => i >= start && i <= end && !(i in tried) &&
        wire.peerPieces.get(i) && (!rank || rank(i))
    }
    ...
    function speedRanker () {
      ...
      const secs = Math.max(1, wire.requests.length) * Piece.BLOCK_LENGTH /
        speed
      let tries = 10
      let ptr = 0
    ...
    function shufflePriority (i) {
      let last = i
      for (let j = i; j < self._selections.length &&
      self._selections[j].priority; j++) {
        last = j
      }
      ...
      for (let i = 0; i < self._selections.length; i++) {
        ...
        if (self.strategy === 'rarest') {
          ...
          while (tries < len) {
            ...
            while (self._request(wire, piece,
  ...
  _rechoke () {
    ...
    if (!this._rechokeOptimisticWire && i < peers.length &&
  ...
  _request (wire, index, hotswap) {
    ...
    wire.request(index, chunkOffset, chunkLength,
      function onChunk (err, chunk) {
        ...
        if (err) {
          self._debug(
            ...
            index, chunkOffset, chunkLength,
            `${wire.remoteAddress}:${wire.remotePort}`,
            err.message,
          )
          isWebSeed ? piece.cancelRemaining(reservation) : piece.cancel(
            reservation)
          onUpdateTick()
          return
        }
  ...
  _debug () {
    ...
    args[0] = `[${this.client
      ? this.client._debugId
      : 'No Client'}] [${this._debugId}] ${args[0]}`
    debug(...args)
  }
   ...
   */
  _drain () {
    this._debug('_drain numConns %s maxConns %s', this._numConns,
      this.client.maxConns)
    if (typeof net.connect !== 'function' || this.destroyed || this.paused ||
      this._numConns >= this.client.maxConns) {
      return
    }
    this._debug('drain (%s queued, %s/%s peers)', this._numQueued,
      this.numPeers, this.client.maxConns)
