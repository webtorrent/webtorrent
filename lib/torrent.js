...
function Torrent (uri, opts) {
  var self = this;
  if (!(self instanceof Torrent)) {
    return new Torrent(uri, opts);
  }
...
Object.defineProperty(Torrent.prototype, 'length', {
  get: function () {
    var self = this;
    return !self.parsedTorrent ? 0 : self.parsedTorrent.length;
  }
});
...
Object.defineProperty(Torrent.prototype, 'timeRemaining', {
  get: function () {
    var self = this,
      remainingBytes = self.length - self.downloaded;
    return self.swarm.downloadSpeed() === 0 ? Infinity : (remainingBytes / self.swarm.downloadSpeed()) * 1000;
  }
});
...
Object.defineProperty(Torrent.prototype, 'progress', {
  get: function () {
    var self = this;
    return !self.parsedTorrent ? 0 : self.downloaded / self.parsedTorrent.length;
  }
});
...
Object.defineProperty(Torrent.prototype, 'ratio', {
  get: function () {
    var self = this;
    return self.uploaded === 0 ? 0 : self.downloaded / self.uploaded;
  }
});
...
Torrent.prototype._onWireWithMetadata = function (wire) {
  ...
  function requestPiece (index) {
    ...
    var endGame = (len === 0 && self.storage.numMissing < 30),
      block = self.storage.selectBlock(index, endGame);
    if (block){
