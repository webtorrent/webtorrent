...
export default class Torrent extends EventEmitter {
  ...
  constructor (torrentId, client, opts) {
    ...
    this.metadata = null
    this._cachedDownloaded = 0
    this._downloadedDirty = true
    /**
  ...
  /**
   * Number of bytes downloaded (verified data + in-progress data)
   * @returns {number} Total bytes downloaded
   */
  get downloaded () {
    if (!this.bitfield) return 0

    // Use cached value if it's still valid
    if (!this._downloadedDirty) {
      return this._cachedDownloaded
    }
    ...
    // Cache the result
    this._cachedDownloaded = downloaded
    this._downloadedDirty = false
    return downloaded
  }
  ...
  _markVerified (index) {
    ...
    this._downloadedDirty = true // Invalidate downloaded cache
