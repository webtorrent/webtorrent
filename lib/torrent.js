  ...
  get: function () {
    ...
    for (var index = 0, len = this.pieces.length; index < len; index++) {
      if (this.bitfield.get(index)) {
        complete += (index !== len - 1) ? this.pieceLength : this.lastPieceLength
      } else {
        var piece = this.pieces[index]
        complete += (piece.length - piece.missing)
      }
    }
    return this.length ? complete / this.length : 0
  }
})
...
Torrent.prototype._onMetadata = function (metadata) {
  ...
  //     }
  //   }), self._onStore.bind(self))
  // })

  process.nextTick(function () { self._onStore() })
...
Torrent.prototype._onPiece = function () {
  var self = this
