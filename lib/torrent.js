...
Torrent.prototype._onWire = function (wire, addr) {
  ...
  if (wire.peerExtensions.dht && self.client.dht && self.client.dht.listening) {
    ...
    wire.on('port', function (port) {
      if (!wire.remoteAddress) return self._debug('ignoring PORT from peer with no address')
      if (port === 0 || port > 65536) return self._debug('ignoring invalid PORT from peer')

      self._debug('port: %s (from %s)', port, wire.remoteAddress + ':' + wire.remotePort)
