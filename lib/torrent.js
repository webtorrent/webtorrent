...
Torrent.prototype._onParsedTorrent = function (parsedTorrent) {
  ...
  if(!self.resumed) self._processParsedTorrent(parsedTorrent)
  ...
  // create swarm or reinitialize it
  if(self.resumed){
    //console.log('reinitalizing swarm')
    self.swarm.destroyed = false
  }else {
...
Torrent.prototype._onPausedTorrent = function (parsedTorrent) {
  var self = this
  debug('on paused')
...
Torrent.prototype._processParsedTorrent = function (parsedTorrent) {
  ...
  //if(this.files) console.log('_processParsedTorrent(): getBuffer type '+(typeof this.files[0].getBuffer))
...
Torrent.prototype._onSwarmListening = function () {
  ...
  //console.log('self.swarm.address().port: '+self.swarm.address().port)
  ...
  //if(self.bitfield) console.log('_onSwarmListening PROGRESS '+self.progress)
...
/**
 * Called when resume event is emitted
 */
Torrent.prototype._onResume = function (parsedTorrent) {
  var self = this
  if (self.destroyed && !self.paused) return
      ...
      self.store.get(index, function (err, buf) {
        ...
        sha1(buf, function (hash) {
          if (hash === self._hashes[index]) {
            ...
            if (!self.pieces[index]) return cb(null)
 ...
 * Pause this torrent.
 */
Torrent.prototype.pause = function (cb) {
  var self = this
  var _parsedTorrent = self.parsedTorrent
  if (self.destroyed || self.paused) return
  debug('on paused')
  
  self.emit('paused')

  self._oldPieces = _.cloneDeep(self.pieces)
  self._oldStore = _.cloneDeep(self.store)
  self._oldBitfield = self.bitfield
  //self._oldFiles = _.cloneDeep(self.files)
  ...
  parallel(tasks, self._onPausedTorrent(_parsedTorrent))
}
...
Torrent.prototype.resume = function () {
  ...
  //console.log('called Torrent.resume()')

  //set resume flag


  //self.files = self._oldFiles
  //self.pieces = self._oldPieces
  //self.store = self._oldStore
  //self.bitfield = self._oldBitfield
