...
export default class Torrent extends EventEmitter {
  ...
  _verifyPiecesUsingBitfield (cb) {
    console.log('Verifying using bitfield')
    const piecesToCheck = new Set()
    let validPieces = 0
    this.files.forEach(file => {
      let checkFile = true
      for (let i = file._startPiece; i <= file._endPiece; ++i) {
        if (this._startupBitfield.get(i)) {
          validPieces++
          if (checkFile) {
            piecesToCheck.add(i)
            checkFile = false
          }
          this._debug('piece verified %s', i)
          this._markVerified(i, true)
        } else {
    ...
    const invalidateStartupBitfield = (cb) => {
      this._startupBitfield = null
      cb(new Error('piece is invalid, force recheck'))
    }
    parallelLimit([...piecesToCheck].map((index) => cb => {
      ...
      this.store.get(index, getOpts, async (err, buf) => {
        ...
        if (err) return invalidateStartupBitfield(cb)

        const hex = await hash(buf, 'hex')
        if (this.destroyed) return cb(new Error('torrent is destroyed'))

        if (hex !== this._hashes[index]) invalidateStartupBitfield(cb)
        cb(null)
      })
    }), FILESYSTEM_CONCURRENCY, (err) => {
      if (err) return this._verifyPieces(cb)
      console.log('verified using bitfield')
      cb(null)
    })
  }

  _verifyPieces (cb) {
    if (this._startupBitfield) {
      return this._verifyPiecesUsingBitfield(cb)
    }
    console.log('Verifying using default')
  ...
  _markVerified (index, isStartup) {
    this.pieces[index] = null
    this._reservations[index] = null
    this.bitfield.set(index, true)
    this.emit('verified', index, isStartup)
  }
      ...
      if (hex === self._hashes[index]) {
        ...
        self.store.put(index, buf, err => {
          if (err) {
            ...
            self._markVerified(index, false)
