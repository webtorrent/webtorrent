...
export default class Torrent extends EventEmitter {
   ...
   * @param client
   * @param {object} opts
   * @param {Array<string>=} opts.announce - Torrent trackers to use (added to list in .torrent or magnet uri)
   * @param {Array<string>=} opts.urlList - Array of web seeds
   * @param {string=} opts.path - Folder to download files to (default=`/tmp/webtorrent/`)
   * @param {boolean=} opts.addUID - (Node.js only) If true, the torrent will be stored in it's infoHash folder to prevent file name collisions (default=false)
   * @param {FileSystemDirectoryHandle=} opts.rootDir - *(browser only)* if supported by the browser, allows the user to specify a custom directory to stores the files in, retaining the torrent's folder and file structure
   * @param {boolean=} opts.skipVerify - If true, client will skip verification of pieces for existing store and assume it's correct
   * @param {Uint8Array|Buffer|Array<number>=} opts.bitfield -  Preloaded numerical array/buffer to use to know what pieces are already downloaded (any type accepted by UInt8Array constructor is valid)
   * @param {FSChunkStore|MemoryChunkStore|Function=} opts.store - Custom chunk store
   * @param {FSChunkStore|MemoryChunkStore|Function=} opts.preloadedStore -  Custom, pre-loaded chunk store
   * @param {number=} opts.storeCacheSlots - Number of chunk store entries (torrent pieces) to cache in memory [default=20]; 0 to disable caching
   * @param {boolean=} opts.destroyStoreOnDestroy - If truthy, client will delete the torrent's chunk store (e.g. files on disk) when the torrent is destroyed
   * @param {object=} opts.storeOpts - Custom options passed to the store
   * @param {boolean=} opts.alwaysChokeSeeders - If true, client will automatically choke seeders if it's seeding. (default=true)
   * @param {function(): object=} opts.getAnnounceOpts - Custom callback to allow sending extra parameters to the tracker
   * @param {boolean=} opts.private - If true, client will not share the hash with the DHT nor with PEX (default is the privacy of the parsed torrent)
   * @param {'rarest'|'sequential'=} opts.strategy - Piece selection strategy, `rarest` or `sequential`(defaut=`sequential`)
   * @param {number=} opts.maxWebConns - Max number of simultaneous connections per web seed [default=4]
   * @param {number|false=} opts.uploads - [default=10]
   * @param {number=} opts.noPeersIntervalTime - The amount of time (in seconds) to wait between each check of the `noPeers` event (default=30)
   * @param {boolean=} opts.deselect - If true, create the torrent with no pieces selected (default=false)
   * @param {boolean=} opts.paused - If true, create the torrent in a paused state (default=false)
   * @param {Array<number>=} opts.fileModtimes - An array containing a UNIX timestamp indicating the last change for each file of the torrent
   */
  ...
  _verifyPiecesUsingBitfield (cb) {
    ...
    this.files.forEach(file => {
      const previouslyVerifiedFilePieces = []
      for (let i = file._startPiece; i <= file._endPiece; ++i) {
        if (this.bitfield.get(i)) {
          previouslyVerifiedFilePieces.push(i)
          if (!piecesToFilesMap.has(i)) {
            piecesToFilesMap.set(i, [])
          }
          piecesToFilesMap.get(i).push(file)
          this.emit('verified', i)
        } else {
          this._markUnverified(i)
          this._debug('piece invalid %s', i)
        }
      }
      const nonOverlappingPiece = previouslyVerifiedFilePieces.find(piece => piecesToFilesMap.get(piece).length === 1)
      if (nonOverlappingPiece) {
        piecesToCheck.add(nonOverlappingPiece)
      } else if (previouslyVerifiedFilePieces.length >= 1) {
        piecesToCheck.add(previouslyVerifiedFilePieces[0])
      }
    })
    const filesToCheck = new Set()
    parallelLimit([...piecesToCheck].map((index) => cb => {
      this._verifyPiece(index, (err, isVerified) => {
        if (err) return cb(err)
        if (!isVerified) {
          const filesOnPiece = piecesToFilesMap.get(index)
          filesOnPiece.forEach(file => {
