...
/**
 * @param {string|Buffer|Object} torrentId    path to the file or folder on filesystem (string)
 ...
 *
 * @param {Object} opts
 * @param {WebTorrent} opts.client            The torrent client
 * @param {Array} opts.announce               Torrent trackers to use (added to list in .torrent or magnet uri)
 * @param {String} opts.path                  Folder to download files to (default=`/tmp/webtorrent/`)
 * @param {Function} opts.store               Custom chunk store (must follow
 *                                            	 [abstract-chunk-store](https://www.npmjs.com/package/abstract-chunk-store) API)
 * @param {Array} opts.urlList                Allow specifying web seeds via `opts` parameter
 * @param {abstract-chunk-store} opts.store   https://github.com/mafintosh/abstract-chunk-store
 *                                            	abstract-chunk-store compliant buffer storage Object
 * @param {String} opts.strategy              Default strategy for downloading torrent chucnks, valid options are:
 *                                            	'sequential' Download from left to right
 *                                            	'rarest' Download the least seeded first? (varification needed)
 * @param {Boolean|Number} opts.uploads       Number of upload slots? (verification needed)
 */
function Torrent (torrentId, opts) {
  ...
  if (torrentId) self._createParsedTorrent(torrentId)
}
...
/**
 * Returnes number of bytes uploaded.
 */
...
/**
 * Returns percentage complete, represented as a number between 0 and 1
 */
...
/**
 * Returns the seed ratio (Uploaded Bytes / torrent size)
 */
...
/**
 * Create a Blob url for the browser to link too. (Browser only)
 */
...
/**
 * Parse a torrentId into a parsed torrent.
 * @param  {String|Buffer|object} torrentId See top documentation
 */
Torrent.prototype._createParsedTorrent = function (torrentId) {
  var self = this
  if (self.destroyed) return
  parseTorrent.remote(torrentId, function (err, parsedTorrent) {
    if (self.destroyed) return
    if (err) return self._onError(err)
    self._initializeParsedTorrent(parsedTorrent)
  })
}

/**
 * Process parsed torrent and initialize swarm
 * @param  {parsedTorrent} parsedTorrent
 */
Torrent.prototype._initializeParsedTorrent = function (parsedTorrent) {
  ...
  // next tick)
  self.swarm.listen(self.client.torrentPort, self._beginDiscoveringPeers.bind(self))
...
/**
 * Assimlate the parsed torrent into this class
 * @param  {parsedTorrent} parsedTorrent
 */
 ...
 */
Torrent.prototype._beginDiscoveringPeers = function () {
  var self = this
  if (self.destroyed) return
  ...
  if (self.info) self._processMetaData(self)

  self.emit('listening', self.client.torrentPort)
}

/**
 * Processes the metadata and prepares files for torrenting
 * @param  {parsedTorrent|Object} metadata
 */
Torrent.prototype._processMetaData = function (metadata) {
  var self = this
  if (self.metadata || self.destroyed) return
  debug('got metadata')

  // TODO: this seems cyclical, we are going from metadata to parsed torrent to metadata again.
  // check if this is necessary.
  var parsedTorrent = this._metaDataToParsedTorrent(metadata)
  self.metadata = self._processParsedTorrent(parsedTorrent)
  ...
  self._prepareFilesForTorrenting()
  ...
  self._verifyExistingTorrentData()

  self.emit('metadata')
}

/**
 * Attempts to transform metadata into a parsed torrent
 */
Torrent.prototype._metaDataToParsedTorrent = function (metadata) {
  var self = this
 ...
 */
Torrent.prototype._prepareFilesForTorrenting = function () {
  var self = this
...
/**
 * Verifies the peices hashes match the hashes
 */
Torrent.prototype._verifyExistingTorrentData = function () {
  var self = this
 ...
 */
Torrent.prototype._setStatusReady = function () {
  var self = this
  if (self.destroyed) return
  debug('on store')
...
/**
 * Setup wire connection and checks dht
 */
Torrent.prototype._setupWireConnection = function (wire, addr) {
  var self = this
  if (addr) {
  ...
  if (wire.peerExtensions.dht && self.client.dht && self.client.dht.listening) {
    ...
    wire.on('port', function (port) {
      ...
      debug('port: %s (from %s)', port, wire.remoteAddress + ':' + wire.Port)
...
/**
 * Over writes our metadata with that from the wire.
 */
Torrent.prototype._fetchMetadataFromWire = function (wire) {
  var self = this
  wire.ut_metadata.on('metadata', function (metadata) {
    debug('got metadata via ut_metadata')
    self._processMetaData(metadata)
  })
  wire.ut_metadata.fetch()
}
/**
 * Sets up a pex connection
 */
Torrent.prototype._connectWireViaPex = function (wire, addr) {
  var self = this
  wire.use(ut_pex())
...
/**
 * Connects a wire, this doesn't seem to be used or is not hit by the unit tests
 * @param  {[type]} wire [description]
 * @param  {[type]} addr [description]
 * @return {[type]}      [description]
 */
Torrent.prototype._connectToWire = function (wire, addr) {
  var self = this
  debug('got wire (%s)', addr || 'Unknown')
  self._setupWireConnection(wire, addr)

  if (!self.metadata) {
    self._fetchMetadataFromWire(wire)
  }

  // use ut_pex extension if the torrent is not flagged as private
  if (typeof ut_pex === 'function' && !self.private) {
    self._connectWireViaPex(wire, addr)
  }
...
/**
 * TODO: This method should probably be refactored, but it's not that complicated
 */
