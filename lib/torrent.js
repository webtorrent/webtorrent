...
export default class Torrent extends EventEmitter {
  ...
  _verifyPiecesUsingBitfield (cb) {
    ...
    this._verifyPiecesUsingHash([...piecesToCheck], (err, piecesVerified) => {
      if (err) {
        return cb(err)
      }
      const filesToCheck = new Set()
      piecesVerified.forEach((isVerified, index) => {
        if (!isVerified) {
  ...
  _verifyPiecesUsingHash (pieces, cb) {
    ...
    parallelLimit(pieces.map((piece, index) => cb => {
      ...
      return cb(null, piecesVerifiedStatus)
    })
  }
