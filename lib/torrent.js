...
export default class Torrent extends EventEmitter {
  ...
  get downloaded () {
    ...
    for (let index = 0, len = this.pieces.length; index < len; ++index) {
      if (this.bitfield.get(index)) { // verified data
        ...
        // A piece is nulled the moment it verifies, while `bitfield` is set
        // from an async store callback, so there is a window in which neither
        // holds. Nothing of it counts as downloaded until the bitfield says so.
  ...
  _updateWire (wire) {
    ...
    function speedRanker () {
      ...
      return index => {
        ...
        // Verified between the caller picking this index and reaching here.
        // Treated exactly as the bitfield case above.
  ...
  _request (wire, index, hotswap) {
    ...
    const piece = self.pieces[index]
    // Same window as above: the bitfield check at the top of this function can
    // pass while the piece is already gone. There is nothing left to reserve.
