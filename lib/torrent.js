...
export default class Torrent extends EventEmitter {
  ...
  _verifyPiecesUsingBitfield (cb) {
    ...
    this.bitfield.buffer = new Uint8Array(this._startupBitfield)
    ...
    parallelLimit([...piecesToCheck].map((index) => cb => {
      this._verifyPiece(index, (err, isVerified) => {
        ...
        if (!isVerified) {
          ...
          return cb(new Error('piece is invalid, force recheck'))
        }
        cb(null)
      })
    }), FILESYSTEM_CONCURRENCY, (err) => {
  ...
  _verifyPieces (cb) {
    if (this._startupBitfield && this._startupBitfield.length === Math.ceil(this.pieces.length / 8)) {
