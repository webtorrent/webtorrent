...
class Torrent extends EventEmitter {
  ...
  _drain () {
    ...
    this._utpConnect(peer, connOpts)
  }

  _utpConnect (peer, connOpts) {
    if (!this.client.utp) return this._tcpConnect(peer, connOpts)
    const self = this
    ...
    conn.once('error', e => { fallbackToTCP(e) })
    conn.once('close', () => { peer.onClose(self) })
  }

  _tcpConnect (peer, connOpts) {
    ...
    conn.once('connect', () => { peer.onConnect() })
    conn.once('error', e => { peer.destroy(e) })
    conn.on('close', () => { peer.onClose(self) })
